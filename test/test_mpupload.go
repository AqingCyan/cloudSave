...
func main() {
	...
	resp, err := http.PostForm(
		"http://localhost:3000/file/mpupload/init",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
	...
	tURL := "http://localhost:3000/file/mpupload/uppart?" +
		"username=admin&token=" + token + "&uploadid=" + uploadID
	_ = multipartUpload(filename, tURL, chunkSize)

	// 4. 请求分块完成接口
	resp, err = http.PostForm(
		"http://localhost:3000/file/mpupload/complete",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
